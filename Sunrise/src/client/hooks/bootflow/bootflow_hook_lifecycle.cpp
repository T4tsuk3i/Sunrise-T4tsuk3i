...
/**
 * Every fix that attaches a detour. `world_step`, the slice-set sample and the lifetime gate
 * probe are absent: they only find addresses to call (or, for the probe, open their own separate
 * transaction), so they cost this group's batch nothing.
 */
...
/**
 * Attaches the boot-step fixes that carry sign-in through to orbit, plus the accessors that find
 * addresses without detouring anything.
 ...
 * @return True when every fix attached and every accessor was found.
 */
bool install() noexcept {
    ...
    // None of these attach anything in this group's own transaction, so they stay outside it.
    ...
    anyFix = anyFix || worldStep || sliceSet || probe;
    g_installed.store(anyFix, std::memory_order_release);
    return everyFix && worldStep && sliceSet && probe;
}

/** Detaches every boot-step fix and accessor, in the reverse order of install. */
...
/** @return True while at least one boot-step fix or accessor is attached. */
